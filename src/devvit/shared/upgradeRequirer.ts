 ...
 */
export async function isAppVersionSupported(throwIfNotSupported = true): Promise<boolean | null> {
  const lowestSupported = await settings.get<string>('upgradeRequirer-semver-lowest-supported');
